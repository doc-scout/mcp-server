...
type AutoIndexer struct {
	sc       FileGetter
	graph    GraphWriter
	cache    corecontent.ContentRepository // nil if content caching disabled
...
func (ai *AutoIndexer) runParsers(ctx context.Context, repos []corescan.RepoInfo) error {
...
func repoServiceName(repo corescan.RepoInfo) string {
...
func (ai *AutoIndexer) upsertParsedFile(_ context.Context, repo corescan.RepoInfo, fileType string, parsed parser.ParsedFile) error {
	autoObs := []string{
		"_source:" + fileType,
	...
	if parsed.EntityName != "" {
		...
		entityExists := slices.ContainsFunc(graph.Entities, func(e coregraph.Entity) bool {
			return e.Name == parsed.EntityName
		})

		if !entityExists {
			if _, err := ai.graph.CreateEntities([]coregraph.Entity{
				{Name: parsed.EntityName, EntityType: entityType, Observations: obs},
			}); err != nil {
				return fmt.Errorf("CreateEntities %q: %w", parsed.EntityName, err)
			}
		} else {
			if _, err := ai.graph.AddObservations([]coregraph.Observation{
	...
	for _, aux := range parsed.AuxEntities {
		...
		auxExists := slices.ContainsFunc(graph.Entities, func(e coregraph.Entity) bool {
	...
	rels := make([]coregraph.Relation, 0, len(parsed.Relations))
	for _, r := range parsed.Relations {
		...
		rels = append(rels, coregraph.Relation{
			From:         from,
			To:           to,
...
func (ai *AutoIndexer) refreshContent(ctx context.Context, repos []corescan.RepoInfo) {
	for _, repo := range repos {
		for _, file := range repo.Files {
...
func (ai *AutoIndexer) archiveStale(_ context.Context, activeRepos map[string]bool) {
	...
	for _, entity := range graph.Entities {
		...
		_, err := ai.graph.AddObservations([]coregraph.Observation{
