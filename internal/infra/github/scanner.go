...
type Scanner struct {
	...
	repos map[string]*corescan.RepoInfo // keyed by repo name

	scanning bool

	lastScanAt time.Time

	onScanComplete func([]corescan.RepoInfo) // called after each full scan completes
...
func New(client *github.Client, org string, scanInterval time.Duration, targetFilesOverride, scanDirs, infraDirs, extraRepos, repoTopics []string, repoRegex *regexp.Regexp, registry *parser.ParserRegistry) *Scanner {
	...
	return &Scanner{
		...
		repos: make(map[string]*corescan.RepoInfo),
...
func (s *Scanner) scanOrg(ctx context.Context) {
	...
	newRepos := make(map[string]*corescan.RepoInfo)
...
func (s *Scanner) TriggerRepoScan(ctx context.Context, owner, repoName string) {
	...
	if len(files) > 0 {
		...
		s.repos[fullName] = &corescan.RepoInfo{
...
func (s *Scanner) scanRepo(ctx context.Context, repoOwner, repoName string) []corescan.FileEntry {

	var entries []corescan.FileEntry
	...
	for _, target := range s.targetFiles {
		...
		if fc != nil {
			...
			entries = append(entries, corescan.FileEntry{
...
func (s *Scanner) scanDocsDir(ctx context.Context, repoOwner, repoName, path string) []corescan.FileEntry {
...
func (s *Scanner) scanInfraDir(ctx context.Context, repoOwner, repoName, path string) []corescan.FileEntry {
...
func (s *Scanner) ListRepos() []corescan.RepoInfo {
	...
	result := make([]corescan.RepoInfo, 0, len(s.repos))
...
func (s *Scanner) SearchDocs(query string) []corescan.FileEntry {
	...
	var results []corescan.FileEntry
