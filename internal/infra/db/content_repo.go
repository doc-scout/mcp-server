...
// corecontent.ContentMatch is a search result from the content cache.


// corecontent.DocRecord is a lightweight document record used by the semantic indexer.
...
func (cc *ContentCache) searchFTS5(query, repoName, fileType string) ([]corecontent.ContentMatch, error) {
	...
	matches := make([]corecontent.ContentMatch, 0, len(rows))

	for _, r := range rows {

		matches = append(matches, corecontent.ContentMatch(r))
...
func (cc *ContentCache) searchLIKE(query, repoName, fileType string) ([]corecontent.ContentMatch, error) {
	...
	for _, row := range rows {
		...
		matches = append(matches, corecontent.ContentMatch{
...
func (cc *ContentCache) ListDocs(repoName string) ([]corecontent.DocRecord, error) {
	...
	out := make([]corecontent.DocRecord, len(rows))

	for i, r := range rows {

		out[i] = corecontent.DocRecord{
