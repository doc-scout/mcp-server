...
func ingestURLHandler(graph GraphStore, cache *memory.ContentCache) func(ctx context.Context, req *mcp.CallToolRequest, args IngestURLArgs) (*mcp.CallToolResult, IngestURLResult, error) {
	...
	return func(ctx context.Context, req *mcp.CallToolRequest, args IngestURLArgs) (*mcp.CallToolResult, IngestURLResult, error) {
		...
		entities, err := graph.CreateEntities([]memory.Entity{
			{
				Name:         entityName,
		...
		if len(entities) == 0 {
			_, err = graph.AddObservations([]memory.Observation{
